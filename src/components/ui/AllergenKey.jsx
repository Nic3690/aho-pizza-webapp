...
const AllergenKey = () => {
  return (
    <div className="mt-6 sm:mt-8 md:mt-10 bg-white bg-opacity-90 text-black p-4 sm:p-6 rounded-lg shadow-md">
        ...
        <div className="grid grid-cols-3 sm:grid-cols-3 md:grid-cols-7 gap-1 sm:gap-2 md:gap-4">
          {Object.entries(allergensList).map(([code, name]) => (
            <div key={code} className="flex flex-col items-center">
              <div className="w-7 h-7 sm:w-8 sm:h-8 md:w-10 md:h-10 rounded-full bg-aho-yellow flex items-center justify-center mb-1 sm:mb-2 shadow-sm">
                <span className="text-[10px] sm:text-xs md:text-sm font-bold text-red-800">{code}</span>
              </div>
              <span className="text-[9px] sm:text-xs md:text-sm text-center">{name}</span>
            </div>
          ))}
        </div>
      </div>
